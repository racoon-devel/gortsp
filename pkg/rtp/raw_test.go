...
func TestRawPacket_ValidateHeader(t *testing.T) {
	...
	var testCases = []testCase{
		...
		{
			raw: RawPacket{0x80, 0x60, 0x23, 0xcc, 0x64, 0x3c, 0xa6, 0x79, 0x6b, 0x8b, 0x45},
		...
		// invalid version
		{
			raw: RawPacket{0x7F, 0x60, 0x23, 0xcc, 0x64, 0x3c, 0xa6, 0x79, 0x6b, 0x8b, 0x45, 0x67},
			err: ErrVersionMismatch{1},
		},
		// simple packet
		{
			raw:  RawPacket{0x80, 0x60, 0x23, 0xcc, 0x64, 0x3c, 0xa6, 0x79, 0x6b, 0x8b, 0x45, 0x67},
			size: HeaderLength,
		},
		// CSRC is missing
		{
			raw: RawPacket{0xA1, 0xe0, 0x24, 0x2f, 0x64, 0x3d, 0x79, 0x69, 0x6b, 0x8b, 0x45, 0x67},
			err: ErrIncompleteHeader{
				Actual:   HeaderLength,
		...
		// incomplete CSRC
		{
			raw: RawPacket{0xA2, 0xe0, 0x24, 0x2f, 0x64, 0x3d, 0x79, 0x69, 0x6b, 0x8b, 0x45, 0x67, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07},
			err: ErrIncompleteHeader{
				Actual:   HeaderLength + 2*4 - 1,
				Expected: HeaderLength + 2*4,
			},
		},
		// simple packet + 2 csrc
		{
			raw:  RawPacket{0xA2, 0xe0, 0x24, 0x2f, 0x64, 0x3d, 0x79, 0x69, 0x6b, 0x8b, 0x45, 0x67, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08},
			size: HeaderLength + 2*4,
		},
		// packet with X bit, but no extension header
		{
			raw: RawPacket{0x90, 0x60, 0x07, 0xb7, 0x2c, 0x7f, 0x54, 0x5f, 0xec, 0x17, 0x7d, 0xc8},
			err: ErrIncompleteHeader{
				Actual:   HeaderLength,
		...
		{
			raw: RawPacket{0x90, 0x60, 0x07, 0xb7, 0x2c, 0x7f, 0x54, 0x5f, 0xec, 0x17, 0x7d, 0xc8, 0xab, 0xac, 0x00, 0x03, 0xe5, 0xd3, 0x03, 0x75, 0x50, 0x1f, 0x38, 0x00, 0x00, 0x05, 0x00},
			err: ErrIncompleteHeader{
		...
		{
			raw:  RawPacket{0x90, 0x60, 0x07, 0xb7, 0x2c, 0x7f, 0x54, 0x5f, 0xec, 0x17, 0x7d, 0xc8, 0xab, 0xac, 0x00, 0x03, 0xe5, 0xd3, 0x03, 0x75, 0x50, 0x1f, 0x38, 0x00, 0x00, 0x05, 0x00, 0x00},
			size: HeaderLength + HeaderExtensionLength + 12,
		},
		// packet with extension header and 2 CSRC
		{
			raw:  RawPacket{0x92, 0x60, 0x07, 0xb7, 0x2c, 0x7f, 0x54, 0x5f, 0xec, 0x17, 0x7d, 0xc8, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0xab, 0xac, 0x00, 0x03, 0xe5, 0xd3, 0x03, 0x75, 0x50, 0x1f, 0x38, 0x00, 0x00, 0x05, 0x00, 0x00},
			size: HeaderLength + 2*4 + HeaderExtensionLength + 12,
		},
	}

	for i, c := range testCases {
		size, err := c.raw.ValidateHeader()
		if c.err == nil {
			assert.NoError(t, err, "testCase : %d", i+1)
			assert.Equal(t, c.size, size, "testCase : %d", i+1)
		} else {
